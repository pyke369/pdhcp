...
void dump_data(char *message, u_char *input, int size, FILE *output, int indent)
{
    char ascii[33], spacer[256];
    int  index1, index2 = 0;

    indent = (indent > (int)(sizeof(spacer) - 1) ? sizeof(spacer) - 1 : indent);
    memset(spacer, ' ', sizeof(spacer));
    spacer[indent] = 0;
    if (message != NULL && *message)
    {
        fprintf(output, "%s%s (%d bytes)\n", spacer, message, size);
    }
    memset(ascii, 0, sizeof(ascii));
    fprintf(output, "%s[00000000]  ", spacer);
    for (index1 = 0; index1 < size; index1 ++)
    {
        if (index1 && !(index1 % 16))
        {
            ...
            if (!(index1 % 32))
            {
                fprintf(output, "%s\n%s[%08x]  ", ascii, spacer, index1);
                memset(ascii, 0, sizeof(ascii));
                index2 = 0;
            }
        }
        fprintf(output, "%02x ", input[index1] & 0xff);
        ascii[index2 ++] = isprint(input[index1] & 0xff) ? input[index1] : '.';
    }
    index1 = strlen(ascii);
    if (index1)
    {
